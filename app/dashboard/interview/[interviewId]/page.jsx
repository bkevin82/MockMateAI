...
import { use } from "react"; // Import `use` to handle promises.
import Webcam from "react-webcam";

function Interview({ params }) {
  // Resolve the `params` promise using `use`.
  ...
  useEffect(() => {
    if (resolvedParams?.interviewId) {
      console.log(resolvedParams.interviewId); // Log the resolved `interviewId`.
      GetInterviewDetails();
    } else {
        ...
        <div className="flex flex-col my-5 gap-5 ">
          <div className="flex flex-col p-5 rounded-lg border gap-5">
            <h2 className="text-lg">
              ...
              {/* {interviewData.jobPsition} */}
              {interviewData?.jobPsition || "N/A"}
            </h2>
            <h2 className="text-lg">
